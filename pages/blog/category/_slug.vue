...
<template>
<div>
	<div data-rsssl=1 class="archive category category-2 wp-custom-logo hide-blogname hide-blogdescription has-sidebar has-avatars">
				...
				<section id="primary" class="content-area">
					<main id="main" class="site-main">
						...
						<header class="page-header">
							<template v-for="n in res_category.list" >
								<h1 :key="n.topics_category_id" v-if="n.topics_category_id == $route.params.slug" class="page-title">カテゴリー: {{n.category_nm}}</h1>
							</template>
						</header><!-- .page-header -->
								...
								<article v-for="n in res_list.list" :key="n.topics_id" id="post-2279" class="post-2279 post type-post status-publish format-standard has-post-thumbnail hentry category-2 tag-4">
									...
									<div class="post-list-content">
										<header class="entry-header">
											<div class="cat-links">
												<nuxt-link :to="`/blog/detail/${n.topics_id}`" rel="category tag">{{n.contents_type_nm}}</nuxt-link>
											</div><!-- .cat-links -->
												<h2 class="entry-title">
													<nuxt-link :to="`/blog/detail/${n.topics_id}`" rel="bookmark">{{n.subject}}</nuxt-link>
												</h2>
											<div class="entry-meta">
												<span class="posted-on">
													<nuxt-link :to="`/blog/detail/${n.topics_id}`" rel="bookmark">
														<time class="entry-date published updated" >{{n.ymd}}</time>
													</nuxt-link>
												</span>
											</div><!-- .entry-meta -->
										</header><!-- .entry-header -->
...
<script>
	export default {
		async asyncData({ $axios, params }) {
			return { 
				res_list: await $axios.$get('/rcms-api/3/blog/list',{params:{cnt:100, contents_type:params.slug}}),
				res_newpost: await $axios.$get('/rcms-api/3/blog/list',{params:{cnt:5}}),
